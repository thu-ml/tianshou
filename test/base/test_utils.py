...
from tianshou.utils.net.discrete import DQN, C51, QRDQN
...
def test_net():
    ...
    assert list(net(data)[0].shape) == expect_output_shape
    num_quantiles = 200
    net = QRDQN(*state_shape, action_shape, num_quantiles)
    expect_output_shape = [bsz, *action_shape, num_quantiles]
    assert list(net(data)[0].shape) == expect_output_shape
