...
class UCTNode(MCTSNode):
    ...
    def backpropagation(self, action):
        ...
        #print("\tU(s,a)  : {}".format(c_puct * self.prior * math.sqrt(np.sum(self.N)) / (self.N + 1.)))
...
class ActionNode(object):
    ...
    def selection(self, simulator):
        ...
        if self.next_state is None: # next_state is None means that self.parent.state is the terminate state
            ...
            return self
        head = time.time()
        self.next_state_hashvalue = simulator.simulate_hashable_conversion(self.next_state)
        self.mcts.hash_time += time.time() - head
        if self.next_state_hashvalue in self.children.keys(): # next state has already visited before
            self.mcts.action_selection_time += time.time() - head
            return self.children[self.next_state_hashvalue].selection(simulator)
        else: # next state is a new state never seen before, so return to _expand for expansion
            self.mcts.action_selection_time += time.time() - head
            return self

    def expansion(self, prior, action_num):
        #print("\t================ expansion state : ", self.next_state[0])
        self.children[self.next_state_hashvalue] = UCTNode(self, self.action, self.next_state, action_num, prior,
...
class MCTS(object):
    ...
    def _expand(self):
        ...
        # next_action.next_state is None means the parent state node of next_action is a terminate node, which means the
        # selection step reaches the terminal node.
