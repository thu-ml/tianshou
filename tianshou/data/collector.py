...
class AsyncCollector(Collector):
    ...
    def _reset_envs_and_hidden_state(
        ...
        for local_id in local_ids_D:
            ...
            if isinstance(self._pre_collect_hidden_state_RH, torch.Tensor):
                self._pre_collect_hidden_state_RH[local_id].zero_()
            elif isinstance(self._pre_collect_hidden_state_RH, np.ndarray):
                self._pre_collect_hidden_state_RH[local_id] = (
                    None if self._pre_collect_hidden_state_RH.dtype == object else 0
                )
            elif isinstance(self._pre_collect_hidden_state_RH, Batch):
                self._pre_collect_hidden_state_RH.empty_(local_id)
    ...
    def collect(
        ...
        batch_of_current_transition_in_all_envs = _get_empty_batch_for_collect(
            self._pre_collect_obs_RO,
        ...
        while True:
            ...
            if num_episodes_done_this_iter:
                ...
                # finished episodes, we have to reset finished envs first.
                obs_reset_DO, info_reset_D = self.env.reset(env_ids=env_ind_global_D,
                                                            **gym_reset_kwargs)
                last_obs_RO[env_ind_local_D] = obs_reset_DO
                last_info_R[env_ind_local_D] = info_reset_D
