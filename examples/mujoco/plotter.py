...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_pattern', type=str, default=r".*/test_rew_\d+seeds.csv$",
                        help='regular expression to determine whether or not to include target csv_file, default to including all test_rew_{num}seeds.csv file under rootdir')
    parser.add_argument('--group_pattern', type=str, default=r"(/|^)\w*?\-v\d(/|$)",
                        help='regular expression to group files in sub-figure, default to grouping according to env_name dir, '' means no grouping')
