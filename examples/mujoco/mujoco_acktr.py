...
def get_args():
    ...
    # this learning rate doesn't really matter in ACKTR algorithm
...
def test_acktr(args=get_args()):
    ...
    optim = KFACOptimizer(actor, critic, lr=args.lr)

    def dist(*logits):
        return Independent(Normal(*logits), 1)

    policy = ACKTRPolicy(actor, critic, optim, dist, discount_factor=args.gamma,
                         gae_lambda=args.gae_lambda, vf_coef=args.vf_coef,
                         ent_coef=args.ent_coef, reward_normalization=args.rew_norm,
                         advantage_normalization=args.norm_adv, action_scaling=True,
    ...
    log_file = f'seed_{args.seed}_{t0}-{args.task.replace("-", "_")}_acktr'
    log_path = os.path.join(args.logdir, args.task, 'acktr', log_file)
