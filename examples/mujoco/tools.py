    ...
    for tfevent_file in tfevent_files:
        ...
        with open(output_file, 'w') as csv_file:
            ...
            for test_rew in test_rews:
                episode_info = {
                    'rew': round(test_rew.value, 4),
                    'env_step': round(test_rew.step, 4),
                    'time': round(test_rew.wall_time - initial_time, 4),
                }
...
def concatenate_csv(
    root_dir='./', output_path=None, pattern=re.compile(r".*/test_rew.csv$")
):
    ...
    with ExitStack() as stack:
        if output_path is None:
            output_path = os.path.join(
                root_dir, "test_rew_{}seeds.csv".format(len(csv_files)))
            print("Concatenating {}.".format(output_path))

        readers = [csv.DictReader(stack.enter_context(
            open(csv_file, newline=''))) for csv_file in csv_files]
        fieldnames = ['env_step', 'rew', 'rew:shaded'] + \
            ['rew:' + p for p in relative_paths]
        ...
        for rows in zip(*readers):
            ...
            line_num += 1
        # summary
        print("{} is generated, concatenating {} data lines from {} files.".format(
            output_path, line_num, len(csv_files)))
...
def analysis_data(root_dir, pattern=r"^.*?(?=/[^/]*seed_\d.*?/)"):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--concatenate_pattern', type=str,
                        default=r"^.*?(?=/[^/]*seed_\d.*?/)")
    args = parser.parse_args()
    args.root_dir = "/mfs/huayu/mujoco_benchmark"
    dirs = list(group_files(find_all_files(args.root_dir, r".*/test_rew.csv$"),
                            args.concatenate_pattern).keys())
    for dir in dirs:
        concatenate_csv(dir)
